...
{
   node* found_element = search_element(header, element->identifier, "main");
   CU_ASSERT(found_element == element);
}
void testSearchingNodeWithWrongIdentifier(void)
{
   node* found_element = search_element(header, "not", "main");
   CU_ASSERT_PTR_NULL(found_element);
}
// Depends on remove function
void testSearchingDeallocatedNode(void)
{  
   //node* found_element = search_element(header, "nodeRemoved");
   //CU_ASSERT_PTR_NULL(found_element);
