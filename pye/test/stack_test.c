...
{
   stack = new_stack();
   stack = insert_scope_on_stack(stack, "firstScope", 0, 0);
   return 0;
}
...
{
   insert_scope_on_stack(stack, "newScope", 0, 0);
   stack_node* node = get_top(stack);
...
int init_suite_pop_element(void)
{
   ...
   insert_scope_on_stack(stack, "insert1", 0, 0);
   insert_scope_on_stack(stack, "insert2", 0, 0);
   insert_scope_on_stack(stack, "insert3", 0, 0);
...
{
   stack = insert_scope_on_stack(stack,"secondScope", 0, 0);
